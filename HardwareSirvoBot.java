...
public class HardwareSirvoBot {
    ...
    //public ElapsedTime runtime = new ElapsedTime(); //Temporarily disabled because it has no use
     ...
     */
    public void driveToObject(double speed, double r, double g, double b) {

        //Define driveInches variable
        double driveInches;
    ...
    public void init(HardwareMap hwMap) {
        ...
        rangeSensor = (ModernRoboticsI2cRangeSensor) hwMap.opticalDistanceSensor.get("range sensor");
        ...
        //Display message that gyro is calibrating
        mode.telemetry.addData("Gyro", "Calibrating...");
        mode.telemetry.update();

        //Test if gyro is still calibrating
        while (!mode.isStopRequested() && gyroSensor.isCalibrating()) {
            ...
            mode.telemetry.addData("Gyro", "Done!");
            mode.telemetry.update();
